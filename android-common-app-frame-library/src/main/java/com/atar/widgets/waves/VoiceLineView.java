package com.atar.widgets.waves;
...
public class VoiceLineView extends View {
    ...
    private void drawMiddleLine(Canvas canvas) {
        ...
        canvas.save();
        canvas.drawRect(0, getHeight() / 2 - middleLineHeight / 2, getWidth(), getHeight() / 2 +
                middleLineHeight / 2, paint);
        canvas.restore();
    }
    ...
    private void drawVoiceLine(Canvas canvas) {
        ...
        for (float i = getWidth() - 1; i >= 0; i -= fineness) {
            ...
            for (int n = 1; n <= paths.size(); n++) {
                float sin = amplitude * (float) Math.sin((i - Math.pow(1.22, n)) * Math.PI / 20
                        - translateX);//+ amplitude * (float) Math.sin(i - n);

                paths.get(n - 1).lineTo(i, (2 * n * sin / paths.size() - 15 * sin / paths.size()
                        + moveY));
            }
        }
        for (int n = 0; n < paths.size(); n++) {
            if (n == paths.size() - 1) {
                ...
                paintVoicLine.setAlpha(0);
//                paintVoicLine.setAlpha(n * 180 / paths.size());
            }
