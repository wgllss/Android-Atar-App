...
public abstract class CommonActivity extends FragmentActivity implements OnOpenDrawerCompleteListener {
    ...
    public void LoadImageView(String imgUrl, ImageView mImageView, int defaultImg,
                              ImageLoadingListener animateFirstListener, int rounde) {
        ...
        DisplayImageOptions options = new DisplayImageOptions.Builder().showStubImage(defaultImg)
                .showImageForEmptyUri(defaultImg).showImageOnFail(defaultImg).cacheInMemory()
                .cacheOnDisc()
                .extraForDownloader(1).displayer(new RoundedBitmapDisplayer(rounde)).bitmapConfig
    ...
    public void LoadImageView(String imgUrl, ImageView mImageView, int defaultImgResId) {
        ...
        DisplayImageOptions options = new DisplayImageOptions.Builder().showStubImage
                (defaultImgResId).showImageForEmptyUri(defaultImgResId).showImageOnFail
                (defaultImgResId).cacheInMemory()
                .cacheOnDisc().build();
        imageLoader.displayImage(imgUrl, mImageView, options);
    }
    ...
    public void LoadImageViewWhitOutCache(String imgUrl, ImageView mImageView, int defaultImg) {
        ...
        mImageView.setImageResource(defaultImg);
        if (imageLoader.getDiscCache() != null && imageLoader.getDiscCache().get(imgUrl) != null
                && imageLoader.getDiscCache().get(imgUrl).getPath() != null
                && imageLoader.getDiscCache().get(imgUrl).getPath().length() > 0 && FileUtils
                .exists(imageLoader.getDiscCache().get(imgUrl).getPath())) {
    ...
    private void onPauseAudio() {
        if (mMediaPlayer != null && mMediaPlayer.isPlaying()) {
            if (mHandlerStatusListener != null) {
                CommonHandler.getInstatnce().handerMessage(mHandlerStatusListener, AUDIO_STOP, 0,
                        0, null);
            }
            mMediaPlayer.stop();
     ...
     */
    @SuppressLint("NewApi")
    public void playAudio(String localPath, AnimationDrawable mAnimationDrawable, ImageView
            imageView, int stopImgResID, final HandlerListener mHandlerListener) {
        ...
        if (mMediaPlayer.isPlaying()) {
            mMediaPlayer.stop();
            if (mHandlerStatusListener != null) {
                CommonHandler.getInstatnce().handerMessage(mHandlerStatusListener, AUDIO_STOP, 1,
                        0, null);// 中途停止播放
            }
            mMediaPlayer.release();
            mMediaPlayer = null;
            ...
            if (strAudioPath != null && strAudioPath.length() > 0 && strAudioPath.equals
                    (localPath)) {
                // 正在播放又再次点击就停止播放
                return;
            }
        ...
        try {
            ...
            mMediaPlayer.start();
            if (mHandlerStatusListener != null) {
                CommonHandler.getInstatnce().handerMessage(mHandlerStatusListener, AUDIO_PLAYING,
                        0, 0, null);
            }
            mMediaPlayer.setOnCompletionListener(new OnCompletionListener() {
                ...
                public void onCompletion(MediaPlayer mp) {
                    ...
                    // }
                    if (mHandlerStatusListener != null) {
                        CommonHandler.getInstatnce().handerMessage(mHandlerStatusListener,
                                AUDIO_STOP, 0, 0, null);
                    }
                    mMediaPlayer.stop();
                    mMediaPlayer.release();
