...
public class PullToRefreshWebView extends PullToRefreshBase<WebView> {
	...
	final class InternalWebViewSDK9 extends WebView {
		...
		private int getScrollRange() {
			if (VERSION.SDK_INT >= VERSION_CODES.KITKAT) {
				return (int) Math.max(0, (float)Math.floor(mRefreshableView.getContentHeight() * mRefreshableView.getScale()) - (getHeight() - getPaddingBottom() - getPaddingTop()) - 10);
			} else {
				return (int) Math.max(0, (float)Math.floor(mRefreshableView.getContentHeight() * mRefreshableView.getScale()) - (getHeight() - getPaddingBottom() - getPaddingTop()));
			}
		}
	}
}
