...
public class WeexTabAdapter extends CommonAdapter<TabMenuItemBean> {
    ...
    private DisplayImageOptions options = new DisplayImageOptions.Builder().showStubImage
            ...
            .extraForDownloader(1).displayer(new RoundedBitmapDisplayer(0)).bitmapConfig(Bitmap
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            mViewHolder.menu_lineLayout_item = (LinearLayout) convertView.findViewById(R.id
					.menu_lineLayout_item);
        ...
        if (info != null) {
            ...
            mViewHolder.menu_title.setTextSize(TypedValue.COMPLEX_UNIT_DIP, info
					.getMenuNameTextSize());
            // mViewHolder.menu_lineLayout_item lp2 整个item背景属性
            FrameLayout.LayoutParams lp2 = new FrameLayout.LayoutParams(FrameLayout.LayoutParams
            ...
            if (info.isShowIcon()) {
                ...
                if (info.getMenuIcon() != null && info.getMenuIcon().length() > 0) {
                    if (getCurrentPostiotn() == position) {
                        mImageLoader.displayImage(info.getSelectMenuIcon(), mViewHolder
								.menu_icon, options);
                    } else {
                        mImageLoader.displayImage(info.getMenuIcon(), mViewHolder.menu_icon,
								options);
                    }
                ...
                lp2.leftMargin = 2 * size;
                lp2.rightMargin = 2 * size;
                // mViewHolder.menu_lineLayout_item.setBackgroundDrawable(parent.getContext()
				// .getResources().getDrawable(R.drawable.corners_transparent1_radius14));
            }
            ...
            if (info.getInfoNum() > 0) {
                if (info.isShowIcon() && info.getOrientation() == LinearLayout.VERTICAL) {
                    mViewHolder.menu_lineLayout_item.setPadding(0, (int) ScreenUtils.getIntToDip
							(5), 0, 0);
                } else {
            ...
            if (info.getHuo_flag_icon_url() != null && info.getHuo_flag_icon_url().length() > 0) {
                mImageLoader.displayImage(info.getHuo_flag_icon_url(), mViewHolder.img_huo_flag,
						options);
                int size = (int) ScreenUtils.getIntToDip(5);
            ...
            if (getCurrentPostiotn() == position) {
                // LoadUtil.setTextColor(parent.getContext(), mViewHolder.menu_title, R.array.common_top_title_bar_bg_color, getSkinType());
            } else {
                // LoadUtil.setTextColor(parent.getContext(), mViewHolder.menu_title, R.array.common_buttom_txt_color, getSkinType());
            }
        }
        return convertView;
    }
